...
// R32 slots: phase="r32", R16: phase="r16", QF: phase="qf", SF: phase="sf", F: phase="f", 3rd: phase="3rd"
const R32_SLOTS=[
  {id:"r32_0",phase:"r32",label:"1°A vs 2°B",home_src:{g:"A",pos:1},away_src:{g:"B",pos:2},date:"2026-06-28",time:"16:00",venue:"Los Ángeles"},
  {id:"r32_1",phase:"r32",label:"1°B vs 2°A",home_src:{g:"B",pos:1},away_src:{g:"A",pos:2},date:"2026-06-28",time:"22:00",venue:"Los Ángeles"},
  {id:"r32_2",phase:"r32",label:"1°C vs 2°F",home_src:{g:"C",pos:1},away_src:{g:"F",pos:2},date:"2026-06-29",time:"14:00",venue:"Houston"},
  {id:"r32_3",phase:"r32",label:"1°F vs 2°C",home_src:{g:"F",pos:1},away_src:{g:"C",pos:2},date:"2026-06-29",time:"22:00",venue:"Guadalajara"},
  {id:"r32_4",phase:"r32",label:"1°E vs 2°I",home_src:{g:"E",pos:1},away_src:{g:"I",pos:2},date:"2026-06-29",time:"17:30",venue:"Foxborough"},
  {id:"r32_5",phase:"r32",label:"1°I vs 2°E",home_src:{g:"I",pos:1},away_src:{g:"E",pos:2},date:"2026-06-30",time:"18:00",venue:"East Rutherford"},
  {id:"r32_6",phase:"r32",label:"1°D vs 2°G",home_src:{g:"D",pos:1},away_src:{g:"G",pos:2},date:"2026-07-01",time:"18:00",venue:"Santa Clara"},
  {id:"r32_7",phase:"r32",label:"1°G vs 2°D",home_src:{g:"G",pos:1},away_src:{g:"D",pos:2},date:"2026-07-01",time:"17:00",venue:"Seattle"},
  {id:"r32_8",phase:"r32",label:"1°J vs 2°K",home_src:{g:"J",pos:1},away_src:{g:"K",pos:2},date:"2026-07-02",time:"19:00",venue:"Miami"},
  {id:"r32_9",phase:"r32",label:"1°K vs 2°J",home_src:{g:"K",pos:1},away_src:{g:"J",pos:2},date:"2026-07-02",time:"22:30",venue:"Kansas City"},
  {id:"r32_10",phase:"r32",label:"1°H vs 2°L",home_src:{g:"H",pos:1},away_src:{g:"L",pos:2},date:"2026-07-03",time:"16:00",venue:"Dallas"},
  {id:"r32_11",phase:"r32",label:"1°L vs 2°H",home_src:{g:"L",pos:1},away_src:{g:"H",pos:2},date:"2026-07-03",time:"19:00",venue:"Philadelphia"},
...
// ─── SCORING ──────────────────────────────────────────────────────────────────
// Grupos: 4pts resultado + 2pts gol local + 2pts gol visitante + 2pts partido exacto
function scoreGroup(pred, off) {
  ...
  let p=0;
  if (oR===pR) p+=4;           // acertó resultado
  if (ph===oh) p+=2;           // gol local exacto
  if (pa===oa) p+=2;           // gol visitante exacto
  if (ph===oh && pa===oa) p+=2; // partido exacto completo
  return p;
}

// Knockout: puntaje según fase
// phase: r32 | r16 | qf | sf | f | 3rd
// off debe tener: home_team, away_team, home, away, pen_home, pen_away, winner
// pred debe tener: home_team, away_team, home, away, pen_home, pen_away, winner
function scoreKO(pred, off, phase) {
  if (!off || !off.home_team || !away_team_from_off(off)) return null;
  let p = 0;

  const pts = KO_PTS[phase];
  if (!pts) return null;

  // Puntos por país clasificado
  const offTeams = [off.home_team, off.away_team].filter(Boolean);
  const predTeams = [pred?.home_team, pred?.away_team].filter(Boolean);

  if (phase === "r32") {
    // r32: 5pts clasificación exacta, 2pts clasificó pero posición incorrecta
    predTeams.forEach((t,i) => {
      if (!t) return;
      if (offTeams[i] === t) p += 5;       // posición exacta
      else if (offTeams.includes(t)) p += 2; // clasificó pero posición incorrecta
    });
  } else {
    // otras fases: solo importa si el equipo está en esa instancia
    predTeams.forEach(t => {
      if (t && offTeams.includes(t)) p += pts.team;
    });
  }

  // Goles por equipo (independiente)
  ...
  // Penales (solo si pred predijo empate a 120' Y hubo penales reales)
...
function away_team_from_off(off) { return off.away_team; }

const KO_PTS = {
  r32: { team:5, goal:5, pen:7 },
  r16: { team:5, goal:5, pen:7 },
  qf:  { team:10, goal:5, pen:7 },
  sf:  { team:15, goal:5, pen:7 },
  f:   { team:20, goal:7, pen:10 },
  "3rd":{ team:20, goal:7, pen:10 },
};

// Score any prediction (group or KO)
...
function SplashView({ctx}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",background:C.bg}}>
      <style>{`
        @keyframes ballFloat{0%,100%{transform:translateY(0px) rotate(0deg);}50%{transform:translateY(-12px) rotate(8deg);}}
        @keyframes ballGlow{0%,100%{filter:drop-shadow(0 0 20px #2280ff88) drop-shadow(0 8px 16px rgba(0,0,0,0.5));}33%{filter:drop-shadow(0 0 28px #00b8d488) drop-shadow(0 8px 16px rgba(0,0,0,0.5));}66%{filter:drop-shadow(0 0 28px #00e5cc88) drop-shadow(0 8px 16px rgba(0,0,0,0.5));}}
      `}</style>
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"24px 20px",display:"flex",flexDirection:"column",gap:14}}>
        <div>
          ...
          <input style={inp} placeholder="nombre@email.com o tu nick" value={identifier} onChange={e=>setIdentifier(e.target.value)}/>
        </div>
        <div>
          ...
          <div style={{position:"relative"}}>
            ...
            <button onClick={()=>setShowPw(p=>!p)} style={{position:"absolute",right:12,top:"50%",
              transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:C.sub2,fontSize:16,padding:0}}>
              {showPw?"🙈":"👁"}
            </button>
          </div>
        </div>
        <GradBtn onClick={login} disabled={loading}>{loading?"Ingresando...":"Entrar"}</GradBtn>
        <button onClick={forgotPw} style={{background:"none",border:"none",color:C.sub2,fontSize:13,cursor:"pointer",padding:"4px 0",fontFamily:font}}>
          Olvidé mi contraseña
        </button>
        <div style={{textAlign:"center",marginTop:8}}>
          <span style={{color:C.sub,fontSize:13}}>¿No tenés cuenta? </span>
          <button onClick={()=>setView("register")} style={{background:"none",border:"none",color:C.accentS,fontSize:13,cursor:"pointer",fontFamily:font,fontWeight:600}}>Registrate</button>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{width,height:120,overflow:"hidden",position:"relative",cursor:"ns-resize"}}>
      ...
      <div style={{overflowY:"scroll",height:"100%",scrollSnapType:"y mandatory",scrollbarWidth:"none",paddingTop:42,paddingBottom:42}}
        ...
        {items.map((item,i)=>(
          <div key={item} style={{height:36,display:"flex",alignItems:"center",justifyContent:"center",scrollSnapAlign:"start",
            fontSize:15,fontWeight:i===idx?700:400,color:i===idx?C.accentS:C.sub,fontFamily:mono,transition:"all 0.1s"}}>
            {item}
          </div>
        ))}
      </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div style={{background:C.surface,borderRadius:"20px 20px 0 0",width:"100%",maxWidth:480,padding:"20px 20px 32px",border:`1px solid ${C.border}`,borderBottom:"none"}}>
            <div style={{display:"flex",alignItems:"center",marginBottom:16}}>
              ...
              <button onClick={()=>setOpen(false)} style={{background:"none",border:"none",color:C.sub,fontSize:22,cursor:"pointer"}}>✕</button>
            </div>
      ...
      <div style={{padding:"20px",display:"flex",flexDirection:"column",gap:14,paddingBottom:40}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <div style={{...inp,width:"auto",padding:"10px 12px",display:"flex",alignItems:"center",gap:6,flexShrink:0,color:C.text,fontSize:14}}>
              <span style={{fontSize:18}}>🇦🇷</span><span style={{color:C.sub2,fontFamily:mono}}>+54</span>
            </div>
  ...
  return(
    <div style={{minHeight:"100vh"}}>
      ...
      <div style={{padding:"12px 16px 0",display:"flex",gap:8}}>
        <button onClick={()=>setView("official")} style={{flex:1,padding:"10px",borderRadius:10,border:`1px solid ${C.border}`,background:C.surface,color:C.accentS,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:font,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>🏟 Resultados oficiales</button>
      ...
      <div style={{padding:"16px",paddingBottom:100}}>
        ...
        {!loading&&myGroups.length===0&&(
          <div style={{textAlign:"center",marginTop:32,color:C.sub,fontSize:13}}>
            <div style={{fontSize:32,marginBottom:12}}>🏟</div><p>No estás en ningún grupo todavía</p>
          </div>
        )}
        {myGroups.map(g=>(
        ...
        {members.map(m=>(
          <div key={m.user_id} style={{...card,marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:14,color:m.user_id===profile.id?C.accentS:C.text}}>
                {m.profiles?.nick||m.profiles?.nombre}{m.user_id===profile.id?" (vos)":""}
              </div>
              {m.profiles?.nombre&&m.profiles?.nick&&(
                <div style={{fontSize:11,color:C.sub,marginTop:1}}>({m.profiles.nombre})</div>
              )}
            </div>
...
function GlobalRankingView({ctx}){
  ...
  async function loadGlobalRanking(){
    ...
    const{data:allPreds}=await supabase.from("predictions").select("user_id,match_id,home,away,pen_home,pen_away,winner,home_team,away_team");
...
function PredictionsView({ctx}){
  ...
  async function loadData(){
    ...
    const{data:myPreds}=await supabase.from("predictions").select("*").eq("user_id",profile.id).eq("group_id",activeGroup.id);
  ...
  function upd(matchId,field,val){setPreds(p=>({...p,[matchId]:{...(p[matchId]||{}),[field]:val}}));}
  ...
  return(
    <div style={{minHeight:"100vh"}}>
      ...
      <Tabs items={[{id:"groups",label:"Grupos"},{id:"knockout",label:"Cruces"}]} active={tab} onSelect={setTab}/>
      {tab==="groups"&&(
        <>
          ...
          <div style={{padding:"10px 14px 100px"}}>
            ...
            {sortedGroupMatches.map(m=>(<PredMatchCard key={m.id} match={m} pred={preds[m.id]||{}} off={official[m.id]||{}} onUpd={(f,v)=>upd(m.id,f,v)}/>))}
          </div>
        </>
      )}
      {tab==="knockout"&&(
        <div style={{padding:"10px 14px 100px"}}>
          <div style={{...card,marginBottom:12,padding:"12px 14px"}}><p style={{color:C.sub,fontSize:13,margin:0,lineHeight:1.6}}>Completá tus predicciones para cada cruce. Los equipos se definen según tus predicciones de grupos.</p></div>
          {["r32","r16","qf","sf","3rd","f"].map(phase=>{
            ...
            return(
              <div key={phase}>
                <div style={{fontSize:11,color:C.accentS,letterSpacing:1,textTransform:"uppercase",marginBottom:8,marginTop:16,paddingLeft:2}}>{phaseLabel}</div>
                {slots.map(slot=>(<KOPredCard key={slot.id} slot={slot} pred={preds[slot.id]||{}} off={official[slot.id]||{}} onUpd={(f,v)=>upd(slot.id,f,v)} phase={phase}/>))}
              </div>
            );
          })}
        </div>
    ...
    <div style={{...card,marginBottom:10}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <ScoreBox value={pred.home??""} onChange={v=>onUpd("home",v)} state={hasOff?(+pred.home===+off.home?"ok":"err"):null}/>
          <span style={{color:C.border2,fontSize:13,fontFamily:mono}}>—</span>
          <ScoreBox value={pred.away??""} onChange={v=>onUpd("away",v)} state={hasOff?(+pred.away===+off.away?"ok":"err"):null}/>
        </div>
...
function KOPredCard({slot,pred,off,onUpd,phase}){
  ...
  return(
    <div style={{...card,marginBottom:10}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:8}}>
        <input style={{...inp,flex:1}} placeholder="Equipo local" value={pred.home_team||""} onChange={e=>onUpd("home_team",e.target.value)}/>
        <input style={{...inp,flex:1}} placeholder="Equipo visitante" value={pred.away_team||""} onChange={e=>onUpd("away_team",e.target.value)}/>
      </div>
      {/* Marcador */}
      <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:8}}>
        ...
        <span style={{fontSize:10,color:C.sub,marginLeft:"auto"}}>{pts?.team}pts/equipo</span>
      </div>
      {showPen&&(
        <div style={{display:"flex",gap:8,padding:"10px",background:C.surface2,borderRadius:8,border:`1px solid ${C.border}`}}>
          <div style={{flex:1}}><div style={{fontSize:10,color:C.sub,marginBottom:4}}>Penales E1</div><input type="number" style={inp} value={pred.pen_home||""} onChange={e=>onUpd("pen_home",e.target.value)} min="0"/></div>
          <div style={{flex:1}}><div style={{fontSize:10,color:C.sub,marginBottom:4}}>Penales E2</div><input type="number" style={inp} value={pred.pen_away||""} onChange={e=>onUpd("pen_away",e.target.value)} min="0"/></div>
          <div style={{flex:1}}><div style={{fontSize:10,color:C.sub,marginBottom:4}}>Ganador</div><input style={inp} value={pred.winner||""} onChange={e=>onUpd("winner",e.target.value)} placeholder="País"/></div>
        </div>
      )}
      {!showPen&&(<div><div style={{fontSize:10,color:C.sub,marginBottom:4}}>Ganador</div><input style={inp} value={pred.winner||""} onChange={e=>onUpd("winner",e.target.value)} placeholder="País"/></div>)}
    </div>
  );
}
...
function RankingView({ctx}){
  ...
  async function loadRanking(){
    const{data:off}=await supabase.from("official_results").select("*");
    const offMap={};off?.forEach(r=>{offMap[r.match_id]=r;});
    const{data:members}=await supabase.from("group_members").select("user_id,profiles(id,nick,nombre)").eq("group_id",activeGroup.id);
...
function AdminView({ctx}){
  ...
  async function save(){
    ...
    setSaving(false);toast$("Resultados guardados ✓");
  }
  ...
  return(
    <div style={{minHeight:"100vh"}}>
      ...
      <Tabs items={[{id:"results",label:"Resultados"},{id:"groups_admin",label:"Grupos"}]} active={tab} onSelect={setTab}/>
      {tab==="results"&&(
        <>
          <Tabs items={Object.keys(GROUPS).map(g=>({id:g,label:g}))} active={ag} onSelect={setAg} small/>
          <div style={{padding:"10px 14px 100px"}}>
...
function ScoreBox({value,onChange,state}){const col=state==="ok"?C.green:state==="err"?C.red:C.border2;return(<input type="number" min="0" max="20" value={value} onChange={e=>onChange&&onChange(e.target.value)} style={{width:42,height:42,textAlign:"center",background:C.surface2,border:`1.5px solid ${col}`,borderRadius:8,color:state==="ok"?C.green:state==="err"?C.red:C.text,fontSize:18,fontWeight:700,fontFamily:mono,outline:"none",MozAppearance:"textfield"}}/>);}
